...
def process_batch(chunks, url, session_dir, base_dir, staging_dir, skip_upload, part_num, session_id):
    if not chunks:
        return
        
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Processing batch of {len(chunks)} chunks (Part {part_num})...")
    ...
    part_suffix = f"_part{part_num}" if part_num > 1 else ""
    merged_file = staging_dir / f"ondemand_{safe_prefix}{part_suffix}.mp4"
    ...
    # Delete the raw chunks immediately to free up space before upload
    for c in chunks:
        if c.exists():
            try:
                c.unlink()
    ...
    if not skip_upload:
        ...
        if upload_script.exists():
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Triggering post-processing AI Document Generation for Part {part_num}...")
...
def smart_monitor(url, skip_upload=False):
    ...
    while True:
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Recording chunk {chunk_num:03d} (Part {part_num})...")
        
        # Start yt-dlp as a background process so we can monitor disk space
        ...
        if force_cut:
            # We cut it due to low disk space. Process the batch immediately!
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Processing batch due to low disk space threshold...")
            process_batch(chunks, url, session_dir, base_dir, staging_dir, skip_upload, part_num, session_id)
            chunks.clear()
            part_num += 1
            
            # Since the stream was force cut but is still live, we immediately continue the loop to record part_num+1
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Resuming live stream capture as Part {part_num}...")
            continue
            
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] yt-dlp exited normally. Checking if stream is still live...")
    ...
    if chunks:
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Broadcast concluded. Processing final batch...")
        process_batch(chunks, url, session_dir, base_dir, staging_dir, skip_upload, part_num, session_id)
